...
export default function EmployeeLeadsPage() {
  ...
  const filteredLeads = myLeads.filter((lead) => {
    ...
    const matchesSource =
      filters.source === "all" || lead.source.toLowerCase() === filters.source;
    const matchesStatus =
      filters.status === "all" ||
      lead.status.toLowerCase().replace(" ", "-") === filters.status;
    const matchesPriority =
      filters.priority === "all" ||
      lead.priority.toLowerCase() === filters.priority;
              ...
              : (
                filteredLeads.map((lead) => {
                  ...
                  return (
                    <tr
                      key={lead.id}
                      className={`transition-colors hover:bg-neutral-800/50 ${getPriorityColor(
                        lead.priority
                      )}`}
                    >
                      <td className="p-4">
                ...
                <div>
                  ...
                  <div className="space-y-3 rounded-lg border border-neutral-800 bg-neutral-800/50 p-4">
                    {editingStatus ? (
                      ...
                      <>
                        <span
                          className={`inline-flex w-full justify-center rounded-full border px-3 py-2 text-sm font-medium ${getStatusColor(
                            selectedLead.status
                          )}`}
                        >
                          {selectedLead.status}
                  ...
                  <div className="space-y-3 rounded-lg border border-neutral-800 bg-neutral-800/50 p-4">
                    ...
                    <div>
                      ...
                      <p className="mt-1 text-sm text-white">
                        {selectedLead.lastContact || "Never"}
                      </p>
                    </div>
                  </div>
                </div>
